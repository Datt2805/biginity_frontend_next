...
export async function getClassroomsForSpeaker(e, successCallback = (data) => { console.log(data) }, errorCallback = (error) => console.error(error)) {
    // 1. Handle Event
    if (e && e.preventDefault) e.preventDefault();

    try {
        // 2. Make the Secure API Call
        // ⚠️ logic: This assumes your backend creates a route that filters by the logged-in user's token
        const data = await makeSecureRequest(`${hostSocket}/api/classrooms/speaker`, "GET");
        ...
        // 5. Run Error Callback
        errorCallback(error);
        throw Error(error.message);
    }
}

// 6. Attach the Handler Helper
getClassroomsForSpeaker.handler = (successCallback, errorCallback) => {
    return (event) => {
        getClassroomsForSpeaker(event, successCallback, errorCallback);
    }
}